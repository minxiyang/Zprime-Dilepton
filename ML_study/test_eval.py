...
sig = df_sig.drop(columns = ["wgt_nominal", "dataset", "dimuon_mass"]).values
sig_wgt = df_sig["wgt_nominal"].values
bkg = df_bkg.drop(columns = ["wgt_nominal", "dataset", "dimuon_mass"]).values
bkg_wgt = df_bkg["wgt_nominal"].values
data = df_data.drop(columns = ["wgt_nominal", "dataset", "dimuon_mass"]).values
...
for key in df_sig.drop(columns = ["wgt_nominal", "dataset"]).columns.to_list():
    ...
    plt.hist(fea_sig[sig_scores<0.2], bins, weights=sig_wgt[sig_scores<0.2], alpha=0.3, label='sig')
    plt.hist(fea_bkg[bkg_scores<0.2], bins, weights=bkg_wgt[bkg_scores<0.2], alpha=0.3, label='bkg')
    plt.hist(fea_data[data_scores<0.2], bins, weights=data_wgt[data_scores<0.2], alpha=0.3, label='data')
    plt.legend(loc='upper right')
