...
train = train_data.drop(columns = ["wgt_nominal", "dataset", "label"]).values
...
val = val_data.drop(columns = ["wgt_nominal", "dataset", "label"]).values
...
for epoch in range(num_epochs):
    ...
    for i in range(train_size):  
        ...
        data = torch.from_numpy(train[i]).to(device)
        label = torch.from_numpy(np.array([train_labels[i]])).to(device)
        ...
        optimizer.step()
        mean_loss += weight_loss.item()
        tot_wgt += w
        if i%4 == 0:
            j = int(i/4)
            val_data = torch.from_numpy(val[j]).to(device)
            val_label = torch.from_numpy(np.array([val_labels[j]])).to(device)
            val_w = val_wgt[j]
            ...
            val_mean_loss += val_loss.item()*val_w
            val_tot_wgt += val_w

        if (i+1) % 10000 == 0:
            print ('Epoch [{}/{}], Step [{}/{}], Train Loss: {:.4f}, Val Loss: {:.4f}' 
                   .format(epoch+1, num_epochs, i+1, total_step, mean_loss/tot_wgt, val_mean_loss/val_tot_wgt))
            mean_loss=0
            tot_wgt=0
            val_mean_loss=0
            val_tot_wgt=0
