...
def plot(data, variable, path_save):
    fig, axs = plt.subplots(
        2, sharex=True, sharey=False, gridspec_kw={"height_ratios": [5, 1]}
    )
...
with ProcessPoolExecutor(max_workers=48) as executor:
    executor.map(
        plot, itertools.repeat(data), variables_plot, itertools.repeat(path_save)
    )
