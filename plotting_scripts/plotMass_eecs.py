...
frames['dielectron_cos_theta_cs'] = frame('cos $\\theta$', [-1, 1], 'Events', [0, 50000], np.linspace(-1,1,25), "Combine", "skyblue", "2018", False, False, True, True)
...
def plot(DY, Zprime, G_RS, variable, path_save):
        ...
        hep.histplot( DY_vals, bins, ax=axs[0], color=frame.color, histtype='fill', label="$\gamma/\mathrm{Z}\\rightarrow e^{+}e^{-}$", edgecolor=(0,0,0))
        ...
        hep.histplot(np.zeros(len(bins)-1), bins, ax=axs[1], histtype='errorbar', color='black')
