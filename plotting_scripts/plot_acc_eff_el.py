...
def df2hist(
    var, df, bins, masscut, iscut=False, flavor="el", iswgt=True, scale=True, nj=-1
):
    cut = 120
    if var == "met":
        cut = 400
...
def plotAcc(dfs, bins, name):
    ...
    for df in dfs:
        ...
        try:
            Yield["0j"] += np.sum(
                df.loc[
                    (df["dielectron_mass"] > cut) & (df["nJets"] == 0), "pu_wgt"
                ].values
            )
            Yield["1j"] += np.sum(
                df.loc[
                    (df["dielectron_mass"] > cut) & (df["nJets"] == 1), "pu_wgt"
                ].values
            )
            Yield["2j"] += np.sum(
                df.loc[
                    (df["dielectron_mass"] > cut) & (df["nJets"] >= 2), "pu_wgt"
                ].values
            )
        except Exception:
            Yield["0j"] = np.sum(
    ...
    for key in df_deno.keys():
        ...
        for i in range(2):
            wgt1 = (
                df_deno[key][i]
                .loc[(df_deno[key][i]["dielectron_mass"] > cut), "pu_wgt"]
                .values
            )
            array1 = (
                df_deno[key][i]
                .loc[(df_deno[key][i]["dielectron_mass"] > cut), "dielectron_mass"]
                .values
            )
            ...
            wgt2 = (
                df_no[key][i]
                .loc[(df_no[key][i]["dielectron_mass"] > cut), "pu_wgt"]
                .values
            )
            array2 = (
                df_no[key][i]
                .loc[(df_no[key][i]["dielectron_mass"] > cut), "dielectron_mass"]
                .values
            )
            ...
            for j in range(3):
                ...
                for k in range(2):
                    ...
                    val += np.sum(
                        ...
                        .loc[
                            (
                                df_mig[str(i) + "g_a" + str(j)][k]["dielectron_mass"]
                                > cut
                            ),
                            "pu_wgt",
                        ]
    ...
    fig.savefig(
        f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/jet_acc_eff_matrix_el_{name}.pdf"
    )
    fig.clf()
...
def plotdf2hist2D(
    ...
    ax.set_xscale("log")
    fig.savefig(
        f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/{name}_{ID}_frac_el_eff.pdf"
    )
    fig.clf()
...
if __name__ == "__main__":
    ...
    for sample in CI_list:
        ...
        labels1 = [
            "dielectron acceptance",
            "electron reconstruction",
            "high-pT electron ID",
            "trigger",
            "dielectron acceptance $\\times$ efficiency",
        ]
        ...
        plots(
            ...
            labels1,
            colors1,
            sample.replace("400", "") + "_acceff_el_inclu",
        )
        ...
        plots(
            ...
            labels1,
            colors1,
            sample.replace("400", "") + "_acceff_el_0j",
        )
        ...
        plots(
            ...
            labels1,
            colors1,
            sample.replace("400", "") + "_acceff_el_1j",
        )
        ...
        plots(
            ...
            labels1,
            colors1,
            sample.replace("400", "") + "_acceff_el_2j",
        )
        ...
        plots(
            ...
            labels2,
            colors2,
            sample.replace("400", "") + "_acceff_bjet_el_inclu",
            "Jet",
        )
