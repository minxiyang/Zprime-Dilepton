...
def df2hist(
    var, df, bins, masscut, iscut=False, flavor="mu", iswgt=True, scale=True, nj=-1
):
    cut = 120
    if var == "met":
        cut = 400
    ...
    if flavor == "mu":
        ...
        for i in range(nj + 1):
            if i == 2:
                dfs.append(
                    df_local[(df_local["nJets_accepted"] >= i) & (df_local["accepted"])]
                )
            else:
                dfs.append(
                    df_local[(df_local["nJets_accepted"] == i) & (df_local["accepted"])]
...
def plotAcc(dfs, bins, name):
    ...
    for df in dfs:
        ...
        try:
            Yield["0j"] += np.sum(
                df.loc[
                    (df["dimuon_mass"] > cut) & (df["nJets"] == 0), "wgt_nominal"
                ].values
            )
            Yield["1j"] += np.sum(
                df.loc[
                    (df["dimuon_mass"] > cut) & (df["nJets"] == 1), "wgt_nominal"
                ].values
            )
            Yield["2j"] += np.sum(
                df.loc[
                    (df["dimuon_mass"] > cut) & (df["nJets"] >= 2), "wgt_nominal"
                ].values
            )
        except Exception:
            Yield["0j"] = np.sum(
        ...
        for i in range(3):
            for j in range(3):
                try:
                    if j < 2 and i < 2:
                        df_mig[str(i) + "g_a" + str(j)].append(
                            df1[(df1["nJets_btag"] == j) & (df1["nJets"] == i)]
                        )
                    elif i == 2 and j < 2:
                        df_mig[str(i) + "g_a" + str(j)].append(
                            df1[(df1["nJets_btag"] == j) & (df1["nJets"] >= i)]
    ...
    for key in df_deno.keys():
        ...
        for i in range(2):
            wgt1 = (
                df_deno[key][i]
                .loc[(df_deno[key][i]["dimuon_mass"] > cut), "wgt_nominal"]
                .values
            )
            array1 = (
                df_deno[key][i]
                .loc[(df_deno[key][i]["dimuon_mass"] > cut), "dimuon_mass"]
                .values
            )
            ...
            wgt2 = (
                df_no[key][i]
                .loc[(df_no[key][i]["dimuon_mass"] > cut), "wgt_nominal"]
                .values
            )
            array2 = (
                df_no[key][i]
                .loc[(df_no[key][i]["dimuon_mass"] > cut), "dimuon_mass"]
                .values
            )
        ...
        err = r * np.sqrt(
            sum(vals1_err) / sum(vals1) ** 2 + sum(vals2_err) / sum(vals2) ** 2
        )
        mean[key] = r
        uncer[key] = err
        mig = {}
        ...
        for i in range(3):
            ...
            for j in range(3):
                ...
                for k in range(2):
                    ...
                    val += np.sum(
                        df_mig[str(i) + "g_a" + str(j)][k]
                        .loc[
                            (df_mig[str(i) + "g_a" + str(j)][k]["dimuon_mass"] > cut),
                            "wgt_nominal",
                        ]
                        .values
                    )
    ...
    hep.histplot(
        ...
        label=[
            "0 b-jet fraction (%s $\pm$ %s)"
            % (format(mean["0j"], ".3f"), format(uncer["0j"], ".4f")),
            "1 b-jet fraction (%s $\pm$ %s)"
            % (format(mean["1j"], ".3f"), format(uncer["1j"], ".4f")),
            "$\geq$ 2 b-jet fraction (%s $\pm$ %s)"
            % (format(mean["2j"], ".3f"), format(uncer["2j"], ".4f")),
        ],
    ...
    axes[1].savefig(
        f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/jet_fraction_{name}.pdf"
    )
    axes[1].clf()
    ...
    axes1[1].savefig(
        f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/jet_acc_eff_migration_{name}.pdf"
    )
    axes1[1].clf()
    ...
    fig.savefig(
        f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/jet_acc_eff_matrix_{name}.pdf"
    )
    fig.clf()


def plotdf2hist2D(
    var, dfs, binx, biny, name, ID, ID_deno="none", iswgt=True, scale=True
):
    cut = 120
    ...
    ax.set_xscale("log")
    fig.savefig(
        f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/{name}_{ID}_frac_eff.pdf"
    )
    fig.clf()
...
if __name__ == "__main__":
    ...
    for sig_name in CI_list:
        files = glob.glob(path + "CI_effv4/" + sig_name + "/*")
    ...
    Jet_eta_bin = np.linspace(-4.0, 4.0, 21)
    ...
    for sample in CI_list:
        ...
        for i in range(2):
            ...
            plotdf2hist2D(
                ["Jet_pt", "Jet_eta"],
                [df_list[sample], df_list[sample.replace("400", "1000")]],
                binx,
                biny,
                sample,
                ID=IDs[i + 1],
        ...
        labels1 = [
            ...
            "high-pT muon ID",
            "trigger",
            "dimuon acceptance $\\times$ efficiency",
        ]
        ...
        labels2 = [
            ...
            "tight ID",
            "b-tagging",
            "total acceptance $\\times$ efficiency",
        ]

        labels_acc = [
            "0 b-jet within acceptance",
            "1 b-jet within acceptance",
            "$\geq$ 2 b-jet within acceptance",
        ]
        ...
        plots(
            ...
            labels1,
            colors1,
            sample.replace("400", "") + "_acceff_inclu",
        )
        ...
        plots(
            ...
            labels2,
            colors2,
            sample.replace("400", "") + "_acceff_bjet_inclu",
            "Jet",
        )
        ...
        axes8[0].set_xticks([10.0, 20.0, 30.0, 50.0, 100.0, 200.0, 500.0, 1000.0])
        plotVar(axes8, pt_Jet[sample], sample.replace("400", "") + "_bjet_pt")
        ...
        axes9[0].set_xticks([-3.6, -2.4, -1.2, 0.0, 1.2, 2.4, 3.6])
        plotVar(axes9, eta_Jet[sample], sample.replace("400", "") + "_bjet_eta")
