...
def df2hist(var, df, bins, masscut, iscut=False, flavor="mu", iswgt=True, scale=True, nj=-1):
    cut = 120
    if var=="met":
        cut = 400
    df = df.compute()
...
def plotAcc(dfs, bins, name):
    ...
    for df in dfs:
        ...
        try:
            Yield["0j"] += np.sum(df.loc[(df["dimuon_mass"] > cut)&(df["nJets"]==0), "wgt_nominal"].values)
            Yield["1j"] += np.sum(df.loc[(df["dimuon_mass"] > cut)&(df["nJets"]==1), "wgt_nominal"].values)
            Yield["2j"] += np.sum(df.loc[(df["dimuon_mass"] > cut)&(df["nJets"]>=2), "wgt_nominal"].values)
        except Exception:
            Yield["0j"] = np.sum(df.loc[(df["dimuon_mass"] > cut)&(df["nJets"]==0), "wgt_nominal"].values)
            Yield["1j"] = np.sum(df.loc[(df["dimuon_mass"] > cut)&(df["nJets"]==1), "wgt_nominal"].values)
            Yield["2j"] = np.sum(df.loc[(df["dimuon_mass"] > cut)&(df["nJets"]>=2), "wgt_nominal"].values)
        ...
        for i in range(3):
            ...
            for j in range(3):
                ...
                for k in range(2):
                    ...
                    val += np.sum(df_mig[str(i)+"g_a"+str(j)][k].loc[(df_mig[str(i)+"g_a"+str(j)][k]["dimuon_mass"] > cut), "wgt_nominal"].values)
    ...
    fig.savefig(f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/jet_acc_eff_matrix_{name}.pdf")
    fig.clf()
...
def plotdf2hist2D(var, dfs, binx, biny,  name, ID, ID_deno="none",iswgt=True, scale=True):
    ...
    ax.set_ylabel("$\eta$")
    ax.set_xscale("log")
    fig.savefig(f"/depot/cms/users/minxi/NanoAOD_study/Zprime-Dilepton/plots/{name}_{ID}_frac_eff.pdf")
    fig.clf()
...
if __name__ == "__main__":
    ...
    for sample in CI_list:
        ...
        labels1 = ["dimuon acceptance", "muon reconstruction", "high-pT muon ID", "trigger", "dimuon acceptance $\\times$ efficiency"]
        ...
        plots(axes2, mass_0[sample], labels1, colors1, sample.replace("400", "")+"_acceff_0j")
        ...
        plots(axes3, mass_1[sample], labels1, colors1, sample.replace("400", "")+"_acceff_1j")
        ...
        plots(axes4, mass_2[sample], labels1, colors1, sample.replace("400", "")+"_acceff_2j")
        ...
        plots(axes5, mass_acc2[sample], labels_acc, colors_acc, sample.replace("400", "")+"_acc_2j", "acc2")
        ...
        plots(axes6, mass_acc1[sample], labels_acc[:-1], colors_acc[:-1], sample.replace("400", "")+"_acc_1j", "acc1")
        ...
        plots(axes7, mass_Jet[sample], labels2, colors2, sample.replace("400", "")+"_acceff_bjet_inclu", "Jet")
