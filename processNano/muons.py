...
def fill_muons(processor, output, mu1, mu2, is_mc, year, weights):
    ...
    recoMassBB = output.loc[((abs(output.mu1_eta < 1.2)) & (abs(output.mu2_eta < 1.2))), "dimuon_mass"].to_numpy()
    recoMassBE = output.loc[((abs(output.mu1_eta > 1.2)) | (abs(output.mu2_eta > 1.2))), "dimuon_mass"].to_numpy()
    genMassBB = output.loc[((abs(output.mu1_eta < 1.2)) & (abs(output.mu2_eta < 1.2))), "dimuon_mass_gen"].to_numpy()
    genMassBE = output.loc[((abs(output.mu1_eta > 1.2)) | (abs(output.mu2_eta > 1.2))), "dimuon_mass_gen"].to_numpy()
