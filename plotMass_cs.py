...
variables_plot = 'dimuon_cos_theta_cs'
frames={}

bins = [120., 129.95474058, 140.73528833, 152.41014904, 165.0535115, 178.74571891, 193.57377942, 209.63191906,  227.02218049,  245.85507143,  266.2502669,   288.3373697,  312.25673399,  338.16035716,  366.21284574,  396.59246138,  429.49225362, 465.12128666,  503.70596789,  545.49148654,  590.74337185,  639.74918031, 692.82032303,  750.29404456,  812.53556599,  879.94040575,  952.93689296,  1031.98888927, 1117.59873655, 1210.310449,   1310.71317017, 1419.4449167,   1537.19663264, 1664.71658012, 1802.81509423, 1952.36973236, 2114.3308507, 2289.72764334, 2479.6746824,  2685.37900061, 2908.14776151, 3149.39656595, 3410.65844758, 3693.59361467, 4000.,4500,5200,6000,7000,8000]


#frames['dimuon_mass'] = frame('$\mathrm{m}(\mu^{+}\mu^{-})$ [GeV]', [120, 6000], 'Events/GeV', [1e-5, 1e7], np.array(bins), "Combine", "skyblue", "2018", True, True, True, True)
frames['dimuon_cos_theta_cs'] = frame('cos $\\theta$', [-1, 1], 'Events', [0, 50000], np.linspace(-1,1,25), "Combine", "skyblue", "2018", False, False, True, True)
...
def plot(DY, other, ttbar, data, Zprime, G_RS, variable, path_save):
        ...
        ttbar_mass = ttbar[variable].compute()
        ...
        ttbar_weight=(ttbar['wgt_nominal'].compute()).values
        ttbar_weight = ttbar_weight[ttbar_tree<500]
        ttbar_tree = ttbar_tree[ttbar_tree<500]
        ttbar_vals, bins  = np.histogram(ttbar_tree, bins=frame.bins, weights=ttbar_weight)
        ttbar_vals2, bins  = np.histogram(ttbar_tree, bins=frame.bins, weights=ttbar_weight**2)
        ttbar_errs = np.sqrt(ttbar_vals2)  
        other_vals = other_vals+ttbar_vals
        other_errs = np.sqrt(other_errs**2+ttbar_errs**2)     
        data_m = (data['dimuon_mass'].compute()).values
        data_vals, bins  = np.histogram(data_tree[data_m>120], bins=frame.bins)
        ...
        hep.histplot( [other_vals, DY_vals], bins, ax=axs[0], color=['orangered',frame.color], histtype='fill', label=["$t\\bar{t}$","$\gamma/\mathrm{Z}\\rightarrow \mu^{+}\mu^{-}$"], edgecolor=(0,0,0),stack=True)
        ...
        axs[0].text(0.1, 0.92, 'SR', verticalalignment='top', horizontalalignment='left', transform=axs[0].transAxes, fontsize=28)
        ...
        fig.savefig(path_save+variable+".pdf")
...
#load data 
    ...
    ttbar_paths = glob.glob('/depot/cms/users/minxi/NanoAOD_study/Zprime-mumu/output/ttbar_lep')
    with ProcessPoolExecutor(max_workers=48) as executor:
