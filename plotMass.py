...
path_data="/depot/cms/users/minxi/NanoAOD_study/Zprime-mumu/output/datav2/*/*.parquet"
...
Zprime2ll_mass = {k: v for k, v in Zprime2ll_mass.items() if k in mpl.rcParams}
...
#                              \sansmath"
#import subprocess
#print(subprocess.check_call(["latex"]))
...
#print(hep.style.CMSTex)
#plt.style.use([hep.style.CMS])
#hep.cms.label()
#hep.cms.text(hep.style.CMSTex)
hep.set_style([Zprime2ll_mass])
...
def plot(MC, data, Zprime, G_RS, variable, path_save):
        ...
        G_RS = G_RS*lumi/140.
        hep.histplot(data_yvals, bins, ax=axs[0], color='black', histtype='errorbar',label="Data", yerr=True)
        hep.histplot(MC_yvals, bins, ax=axs[0], color=frame.color, histtype=htype, label="$\gamma/\mathrm{Z}\\rightarrow \mu^{+}\mu^{-}$", edgecolor=(0,0,0))
        ...
        hep.histplot(rvals, bins, ax=axs[1], histtype='errorbar', color='black', yerr=True)
        #lumi=lumis[frame.year]
        #Zprime = Zprime*lumi/140.
        #G_zy = G_zy*lumi/140.
        #print(Zprime)
